...
// Global Neon Green Shader Source
export const neonGreenShaderSource = `#version 300 es
precision highp float;
out vec4 O;
...
void main(void) {
	...
	for (float i=1.; i<12.; i++) {
		...
		float d=length(p);
    // Darker Pure Green
		col+=.0008/d*(cos(sin(i)*vec3(0.0, 1.0, 0.0))+1.0);
		float b=noise(i+p+bg*1.731);
		col+=.001*b/length(max(p,vec2(b*p.x*.02,p.y)));
		col=mix(col,vec3(0.0, bg*.10, 0.0),d);
	}
	O=vec4(col * 0.6, 1.0);
}`;

export const ShaderBackground: React.FC = () => {
  const canvasRef = useShaderBackground(neonGreenShaderSource);
